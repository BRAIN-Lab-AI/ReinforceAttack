...
def configure_PA(t_name, t_labels, 
                 ...
                 TPA_n_agents=10, run_tag = 'new'
                 ):
    
    # NEW PARAMS
    PA_cfg.run_tag = run_tag
    ...
    # Cross-shaped configuration (cross-only)
    PA_cfg.cross_thickness = 1
    PA_cfg.cross_min_len = 2
    PA_cfg.use_texture_wrap = True



    # Attack's shared params
    PA_cfg.target = target  # targeted or non-targeted attack
    PA_cfg.n_occlu = n_occlu  # num of patches each agent can put on (default: 1)
    ...
    PA_cfg.sigma = 400
    PA_cfg.sigma_sched = []  # sigma schedule for the multiple occlusions (default: n-occlu * sigma
