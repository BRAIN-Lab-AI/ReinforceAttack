...
class TPA():
    ...
    def attack(self, model, input_tensor, label_tensor, target, input_name='temp', target_in_dict_mapping=None, run_tag = 'new'):
    ...
    def calculate_area(p_image, combos):
        """
        Compute union occlusion fraction using the **equal-area** rendering used by the agent.
        In 'mixed' mode, 'size_occlu' given to combo_to_image is the base square side L and
        circle/triangle parameters are derived as: r = round(L/sqrt(pi)), l = round(sqrt(2)*L).
        """
        import numpy as np
        import torch
        H, W = p_image.size()[-2:]
        ...
        agents = len(combos)
        occ_per_agent = int(getattr(PA_cfg, 'n_occlu', 1))
        num_occlu_total = agents * occ_per_agent

        shape = getattr(PA_cfg, 'patch_shape', None)
        area_sched = getattr(PA_cfg, 'area_sched', [getattr(PA_cfg, 'area_occlu', 0.04)] * agents)

        def derive_size(area_frac):
            A = H * W * float(area_frac)
            if (shape is None) or (shape == 'mixed') or (shape == 'square'):
                # base square side L (safe for mixed)
                return int(torch.tensor([A]).sqrt_().floor_().long().item())
            elif shape == 'circle':
                return int(np.floor(np.sqrt(A / np.pi)))   # radius
            elif shape == 'triangle':
                return int(np.floor(np.sqrt(2.0 * A)))     # side
            else:
                return int(torch.tensor([A]).sqrt_().floor_().long().item())

        # one size PER OCCLUDER
        pre_size_occlu = []
        for i in range(agents):
            s = derive_size(area_sched[i] if i < len(area_sched) else getattr(PA_cfg, 'area_occlu', 0.04))
            if isinstance(s, tuple):   # rectangle (rare)
                pre_size_occlu.extend([s] * occ_per_agent)
            else:
                pre_size_occlu.extend([int(s)] * occ_per_agent)

        out = TPA_agent.combo_to_image(
            combo=torch.cat(combos, dim=1),
            num_occlu=num_occlu_total,
            mask=None,
            image_batch=p_image.unsqueeze(0),
            noises=None,                   # mask-only path
            size_occlu=pre_size_occlu,
            output_p_masks=True
        )

        p_mask = out[1] if isinstance(out, (tuple, list)) else out
        p_mask = p_mask.squeeze(0)         # (H, W)
        return p_mask.float().sum() / (H * W)
